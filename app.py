...
def fetch_prices(s, e):
    # Käytetään API:n parametreja: 
    # quarter (15min), fix (snt/kWh), vat (sis. ALV)
    url = f"https://sahkotin.fi/prices?quarter&fix&vat&start={s.isoformat()}&end={e.isoformat()}"
    try:
        ...
        # Koska käytämme 'fix' ja 'vat', 'value' on nyt suoraan snt/kWh sis. ALV
        df["snt_per_kwh"] = df["value"]
...
st.info("💡 Käytössä aito 15 min varttihinnoittelu. Hinnat snt/kWh sis. ALV.")
...
if st.button("Laske kustannukset", type="primary", use_container_width=True):
    with st.spinner("Haetaan varttihintoja..."):
        df = fetch_prices(start_dt, end_dt)
        if df.empty and sopimus == "Pörssisähkö":
            st.error("Datan haku epäonnistui. Tarkista aikaväli.")
        else:
            ...
            m2.metric("Keskihinta (sis. kulut)", f"{avg_total:.2f} snt/kWh")
            m3.metric("Kesto", f"{int(h_kesto)}h {int((h_kesto*60)%60)}min")

            if not df_f.empty:
                st.subheader("Hinnan kehitys (sis. ALV)")
                graph_df = df_f.copy()
                ...
                v_porssi = graph_df.pivot(index='hour_group', columns='min', values='snt_per_kwh')
